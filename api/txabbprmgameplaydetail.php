...
/**
 * Service API:  txabbprmgameplaydetail
 * Purpose: Sync the Practice Mode 'gameplaydetail' data to the ABS 
 * File name: txabbprmgameplaydetail.php
 ...
 */
    ...
    if(count($jsondata_array,1) == 0) {
        ...
        $em = new exceptionMgr(" ");
        $em->logInfo("txabbprmgameplaydetail: Error: Received no input JSON data."); 
    }
    else {
        ...
        foreach($jsondata_array as $data) {
            ...
            if($data) {
                ...
                if(!$childexists) {
                    ...
                    $em = new exceptionMgr(" ");
                    $em->logInfo("txabbprmgameplaydetail: Error: No account exists with the given name and deviceid (".$avatarname.",".$deviceid.")");
                }
                else {
                    ...
                    if($childid == null) {
                        ...
                        $em = new exceptionMgr(" ");
                        $em->logInfo("txabbprmgameplaydetail: Error: Could not retrive the Child ID for the given name and deviceid (".$avatarname.",".$deviceid.")");
                    }
                    else {
                        ...
                        $objGameplaydetail->setChildName($avatarname);
                        $objGameplaydetail->setChildId($childid);
                        $objGameplaydetail->setDeviceId($deviceid);
                        ...
                        if($rtn) {
                            ...
                            $em = new exceptionMgr(" ");
                            $em->logInfo("txabbprmgameplaydetail: Error: Failed to save Gameplaydetail Data."); 
                        }
                    }
                }
            }
                ...
                $em = new exceptionMgr(" ");
                $em->logInfo("txabbprmgameplaydetail: Error: Data missing for this JSON object."); 
             }
