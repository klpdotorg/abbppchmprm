...
class logMgr {
    ...
    private function __construct($loglvl, $logfname, $rolloversize, $maxarchives) {
		...
		$this->logfilename = $_SESSION['ABSAPP_BASE_DIR']."/logfiles/".$logfname;
		  ...
		  try {
			  ...
			  if($this->logrolloversize != 0) {
				  ...
				  if(file_exists($this->logfilename)) {
				      if(!($fsize = filesize($this->logfilename))) {
                      throw new exceptionMgr("Failed with function filesize() in writeToLog.");
				    }
				  }
    ...
    private function rollover() {
          ...
          try {
              ...
              if(file_exists($fname)) {
				  //if(!unlink($this->logfhandle)) {  // Delete the oldest logfile. Changed to use $fname instead of logfhandle - suresh (18/3/2019)
				  if(!unlink($fname)) {  // Delete the oldest logfile
